...
class RamKsaDrForm(http.Controller):
    @http.route('/api/ram_ksa_dr_form', type='json', auth='api_key', cors='*', csrf=False)
    def ram_ksa_dr_form(self):
        r = http.request.httprequest.json

        # https://ramclinics.net/book-an-appointment
        # RamKsa_Dr_formV2.json
        ...
        lead = http.request.env['crm.lead'].sudo().create({
            ...
            'source_id': http.request.env.ref('ramcrm.dr_form').id,
