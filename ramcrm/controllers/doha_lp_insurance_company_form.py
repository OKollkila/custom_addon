...
class DohaLpInsuranceCompany(http.Controller):
    @http.route('/api/doha_lp_insurance_company_form', type='json', auth='api_key', cors='*', csrf=False)
    def doha_lp_insurance_company_form(self):
        r = http.request.httprequest.json

        #     Link_Page EX=> https://doha.medical-clinics.net/insurance
        # RamLP_InsuranceCompany_Form

        required_fields = [
            # 'BU', # Doha Medical Center
            'Lead_Name', # string Of Name
            'Topic', # string Of service_name
        ...
        lead = http.request.env['crm.lead'].sudo().create({
            ...
            'bu': r.get('BU'),
            'source_id': http.request.env.ref('ramcrm.doha_lp_insurance_company').id,
