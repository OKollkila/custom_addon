...
class RamKsaLpForOffer(http.Controller):
    @http.route('/api/ram_ksa_lp_for_offer', type='json', auth='api_key', cors='*', csrf=False)
    def ram_ksa_lp_for_offer(self):
        r = http.request.httprequest.json

        # EX: https://ram.medical-clinics.net/Derma-opening-offers-Clinizone
        # RamKsa_LP_For_OfferV2.json
        ...
        lead = http.request.env['crm.lead'].sudo().create({
            ...
            'source_id': http.request.env.ref('ramcrm.lp_for_offer').id,
